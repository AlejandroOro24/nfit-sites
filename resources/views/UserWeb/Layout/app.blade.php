...
<head>
    ...
    <link rel="stylesheet" href="{{asset('/user/css/app.css?v=2')}}">
...
<body>
    ...
    <div class="app" id="app_2">
        <!-- Toast -->
        <alert-toast></alert-toast>
        <!-- Navbar -->
        
        <sidebar-menu :auth_user='@json($safeUser)':roles='@json(\App\Models\Role::humanList())'></sidebar-menu>
        ...
        <main @if(Request::segment(2) === 'videos') class="dark full-spacing" @endif>
            <!-- Headbar -->
                <header-bar :auth_user='@json($safeUser)'  :sport_center='@json($sport_center)'></header-bar>
